...
This requires some familiarity with R, but hopefully not too much, plus a little understanding of ggplot2. (Also, I am a bit of a novice to coding, so there are definitely multiple ways of accomplishing similar goals. Some much more elegant than these. But I don't know them yet.)

## Dependecies
These packages are what I run. Some of them are not entirely necessary, but make plotting easier/nicer. I have marked them. The order of loading can also matter.
```{r}
library(rstatix)
library(openxlsx)
library(plyr)
...
2. A plate template that has columns for well name, strain/genotype, condition, replicate (within the plate), and plate name (helpful when run in a series)
   + Empty wells should have "blank" as their sample name.
   + The way the code is written (and to make your life easier later), *do not use periods or dots in your names* for strain, condition, etc. Hypens and underscores are OK. Some of the formatting tricks rely on the dots as delimitters between items.
...
For the tempalte, I use plate_template.xlsx. I keep a clean one, and then make a new one for each plate. (In the process of filling them out, I resort the columns to speed up numbering replicates. BUT because I kept well names as one unit (A12 verus row A column 12), Excel has trouble re-ordering them.) When running a series of plates, keep the naming (including capitalization) of strains and conditions the same between them. It can also be helpful to name the plates themselves sequentially with the experiment name rather than just with a date.  
...
The `plot_from_melted_plate_facted` and `plot_from_melted_plate` functions are both simple functions, meant to be a fast way to look at each plate. (They can take more than 1 plates-worth of data in one data frame, but formatting and customiazability are limited.) The first returns a single plot faceted by whatever your different conditions are, the second returns 1 plot per condition. Both require you to give them the data frame (no path needed this time), a (named) color vector for coloring by strain, a title in string format, and the type of scaling on the y-axis you would like ("straight", "log2", "log10", "natural_log".)
...
```{r}
means.plot + facet_wrap(condition~.)
```
...
## Across all samples, within a conition. (For all conditions.)
