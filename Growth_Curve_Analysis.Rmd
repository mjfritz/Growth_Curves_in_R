...
Troubleshooting is a huge part of programming. Don't be 
...
```{r}
library(rstatix)
library(openxlsx) # Needed for Parsing, those base R has caught up a bit
library(janitor)
library(reshape2)
...
To store the fits in a matrix/data table format, we extract them with `results()`.
The `View()` function is similar to clicking the varialbe name in the Environment window. If you see numbers that are bumping up against your limits, go back and try tweaking the other intial parameters.
```{r}
...
```{r}
coef.interest <- coef.interest <- c("auc24", "auc36", "mumax", "K", "lambda")
df.measures.nest <- df.measures %>%
  select(strain, Buffer, Ions, condition, sample_name, auc24, auc36, mumax, K, lambda) %>%
  pivot_longer(cols = all_of(coef.interest), names_to = "measures", values_to = "values") %>%
  group_by(measures) %>%
  nest() %>%
  ungroup(measures)
```
...
We can look at the overall results of the anovas. They are going to display in the same order as the rows of the data frame. (So here, auc24 is first, lambda is last.)
```{r}
df.aovs$measures
lapply(df.aovs$aovs, summary)
```
...
plotdf.data <- df.measures %>% ungroup() %>%
  pivot_longer(cols = coef.interest, names_to = "measures", values_to = "values")
```
...
measureplot.conditions <- ggplot(data = df.cld.plot, aes(x = strain, color = strain)) +
  ...
  # This is me highlighting a sepecific plot
...
Now we only have 4 conditions to color over.

```{r}
fourcolor <- paletteer_d("LaCroixColoR::PassionFruit", 5)[c(1,3,4,5)]
names(fourcolor) <- unique(interaction(df.cld.byStrains$Buffer, df.cld.byStrains$Ions))
```
...
df.cld.byStrain.plot <- full_join(df.cld.byStrains, df.maxes,
                                 by = c("strain", "measures", "Buffer", "Ions")) %>%
  add_label_ypos
